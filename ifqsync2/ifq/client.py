...
URL_SEARCH = 'https://shop.ilfattoquotidiano.it/wp-content/plugins/ifq-searchpdf/get_risultatopdf.php?id=%s-%s-%s'
URL_DOWNLOAD = 'http://pdf.ilfattoquotidiano.it/openpdf/?n=%s%s%s'
...
class Client(object):
    ...
    def _build_search_url(self, pub_date):
        return URL_SEARCH % (pub_date.day, pub_date.month, pub_date.year)

    def _build_download_url(self, pub_date):
        return URL_DOWNLOAD % (pub_date.year, pub_date.month, pub_date.day)